	...
	return ModalCard("add-consultant-modal",
		...
		h.Section(h.Class("modal-card-body"),
			h.Div(h.Class("panel-block"),
				h.Form(h.ID(formName), h.Class("content"),
					// HTMX for submit : call consult update with current consultant id
					x.Trigger("submit"), x.Post(fmt.Sprintf("/action/consult/update/%s", csl.Id)),
					x.Swap("outerHTML"), x.Target(fmt.Sprintf("#consultant-%s", csl.Id)),
					h.Div(h.Class("field"),
		...
		h.Footer(h.Class("modal-card-foot"),
			h.Div(h.Class("buttons"),
				...
				h.Button(h.Class("button"), g.Text("Annuler"),
					...
					x.Get("/action/closemodal"), x.Swap("outerHTML"), x.Target("closest .modal"),
				),
			),
		),
	)
		...
		h.Section(h.Class("modal-card-body"),
			h.Div(h.Class("panel-block"),
				h.Form(h.ID(formName), h.Class("content"),
					...
					x.Post("/action/consult/add"), x.Swap("innerHTML"), x.Trigger("submit"), x.Target("#consultant-list"),
					h.Div(h.Class("field"),
						h.Label(h.Class("label"), g.Text("Prénom")),
						h.Div(h.Class("control"),
