...
const (
	...
	TimeDayLayout        string = "2006-01-02"
	TimeDisplayLayout    string = "02/01/2006"
...
func ParseDate(d string) (Date, error) {
	date, err := time.Parse(TimeDayLayout, d)
	if err != nil {
		return Date{}, err
	}
...
func (d Date) String() string {
	return time.Time(d).Format(TimeDayLayout)
}
...
func (d Date) ToDisplay() string {
	return time.Time(d).Format(TimeDisplayLayout)
}
...
// GetFirstOfMonth returns Month' first of given date (YYYY-MM-01 format)
func GetFirstOfMonth(d string) string {
	return DateFrom(d).GetMonth().String()
}

// GetFirstOfQuarter returns Quarter' first of given date (YYYY-MM-01 format)
func GetFirstOfQuarter(v string) string {
